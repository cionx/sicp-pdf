...
\begin{itemize}[leftmargin=*]
	...
	\item
		...
		\footnote{
			As mentioned in \cref{Section 2.3.1}, the evaluator sees a quoted expression as a list beginning with \code{quote}, even if the expression is typed with the quotation mark.
...
\begin{exercise}
	...
	special forms \code{and} and \code{or} from \cref{Chapter 1}:
	\begin{itemize}
...
\begin{exercise}
	...
	\begin{scheme}
	  ((lambda (⟨~\var{var}\ind{1}~⟩ … ⟨~\var{var}\ind{n}~⟩)
	     ...
	     ⟨~\var{exp}\ind{n}~⟩
	\end{scheme}
	Implement a syntactic transformation \code{let->combination} that reduces evaluating \code{let} expressions to evaluating combinations of the type shown above, and add the appropriate clause to \code{eval} to handle \code{let} expressions.
\end{exercise}
...
\begin{exercise}
	...
	If we have already implemented \code{let} (\cref{Exercise 4.6}) and we want to extend the evaluator to handle \code{let*}, is it sufficient to add a clause to \code{eval} whose action is
	\begin{scheme}
...
\begin{exercise}
	...
	For example, the iterative Fibonacci procedure (\cref{Section 1.2.2}) can be rewritten using named \code{let} as follows:
	\begin{scheme}
	  (define (fib n)
	    (let fib-iter ((a 1)
	                   (b 0)
	...
	\end{scheme}
	Modify \code{let->combination} of \cref{Exercise 4.6} to also support named \code{let}.
\end{exercise}
