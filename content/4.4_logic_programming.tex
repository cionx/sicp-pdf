...
The nondeterministic program evaluator of \cref{Section 4.3} also moves away from the view that programming is about constructing algorithms for computing unidirectional functions.
...
In a procedural language such as Lisp, we could define \code{append} in terms of the basic list constructor \code{cons}, as we did in \cref{Section 2.2.1}:
\begin{scheme}
  (define (append x y)
...
\input{content/4.4.1_deductive_information_retrieval.tex}
\input{content/4.4.2_how_the_query_system_works.tex}
\input{content/4.4.3_is_logic_programming_mathematical_logic.tex}
\input{content/4.4.4_implementing_the_query_system.tex}
