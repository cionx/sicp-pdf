...
The actual shape of the drawing depends on the frame---all four images in \link{Figure 2.10} are produced by the same \code{wave} painter, but with respect to four different frames.
...
The four images in \link{Figure 2.11} are drawn with respect to the same four frames as the \code{wave} images in \link{Figure 2.10}.
...
\link{Figure 2.12} shows the drawing of a painter called \code{wave4} that is built up in two stages starting from \code{wave}:
\begin{scheme}
...
Here’s one that makes painters split and branch towards the right as shown in \link{Figure 2.13} and \link{Figure 2.14}:
\begin{scheme}
