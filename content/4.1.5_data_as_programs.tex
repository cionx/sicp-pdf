...
For example, if we feed our evaluator the definition of \code{factorial}, as shown in \cref{Figure 4.3}, the evaluator will be able to compute factorials.
...
\footnote{
	Warning:
	This \code{eval} primitive is not identical to the \code{eval} procedure we implemented in \cref{Section 4.1.1}, because it uses \emph{actual} Scheme environments rather than the sample environment structures we built in \cref{Section 4.1.3}.
	...
	Similarly, the \code{apply} primitive we saw earlier is not identical to the metacircular \code{apply}, because it uses actual Scheme procedures rather than the procedure objects we constructed in \cref{Section 4.1.3} and \cref{Section 4.1.4}.
}
Thus,
\begin{scheme}
