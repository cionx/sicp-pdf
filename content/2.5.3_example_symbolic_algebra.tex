...
A variable will be just a symbol, so we can use the \code{same-variable?}  procedure of \cref{Section 2.3.2} to compare variables.
...
\footnote{
	This operation is very much like the ordered \code{union-set} operation we developed in \cref{Exercise 2.62}.
...
\footnote{
	...
	If we did not want to make such a guarantee, we could have implemented \code{adjoin-term} to be similar to the \code{adjoin-set} constructor for the ordered-list representation of sets (\cref{Exercise 2.61}).
}
\begin{scheme}
...
\end{scheme}
where \code{=zero?} is as defined in \cref{Exercise 2.80}.
...
\end{scheme}
where \code{remainder-terms} picks out the remainder component of the list returned by the term-list division operation \code{div-terms} that was implemented in \cref{Exercise 2.91}.
...
\begin{exercise}
	...
	Now define \( Q_1 \) to be the product of \( P_1 \) and \( P_2 \) and \( Q_2 \) to be the product of \( P_1 \) and \( P_3 \), and use \code{greatest-common-divisor} (\cref{Exercise 2.94}) to compute the \acronym{GCD} of \( Q_1 \) and \( Q_2 \).
...
We can solve the problem exhibited in \cref{Exercise 2.95} if we use the following modification of the \acronym{GCD} algorithm (which really works only in the case of polynomials with integer coefficients).
...
\begin{exercise}
	...
	\begin{enumerate}[label = \alph*., leftmargin = *]
		...
		\item
			...
			Modify \code{gcd-terms} to use \code{pseudoremainder-terms}, and verify that \code{greatest-common-divisor} now produces an answer with integer coefficients on the example in \cref{Exercise 2.95}.
...
\begin{itemize}
	...
	\item
		Compute the \acronym{GCD} of the numerator and denominator, using the version of \code{gcd-terms} from \cref{Exercise 2.96}.
...
\footnote{
	One extremely efficient and elegant method for computing polynomial \acronym{GCD}s was discovered by Richard \cref{Zippel (1979)}.
	The method is a probabilistic algorithm, as is the fast test for primality that we discussed in \cref{Chapter 1}.
	Zippel’s book (\cref{Zippel 1993}) describes this method, together with other ways to compute polynomial \acronym{GCD}s.
}
