...
As an example, compare the \code{length} procedure of \cref{Section 2.2.1} with the \code{count-leaves} procedure, which returns the total number of leaves of a tree:
\begin{scheme}
...
\begin{exercise}
	\label{Exercise 2.27}
	Modify your \code{reverse} procedure of \cref{Exercise 2.18} to produce a \code{deep-reverse} procedure that takes a list as argument and returns as its value the list with its elements reversed and with all sublists deep-reversed as well.
	For example,
	\begin{scheme}
...
For instance, the \code{scale-tree} procedure, analogous to \code{scale-list} of \cref{Section 2.2.1}, takes as arguments a numeric factor and a tree whose leaves are numbers.
...
\begin{exercise}
	\label{Exercise 2.30}
	Define a procedure \code{square-tree} analogous to the \code{square-list} procedure of \cref{Exercise 2.21}.
...
\begin{exercise}
	\label{Exercise 2.31}
	Abstract your answer to \cref{Exercise 2.30} to produce a procedure \code{tree-map} with the property that \code{square-tree} could be defined as
	\begin{scheme}
